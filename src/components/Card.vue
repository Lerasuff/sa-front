<template>
  <div class="card-container" :class="`card_${card.num}`" :draggable="card.draggable" @dragstart="$emit('ondragstart', $event, card)" @drag="$emit('ondrag', $event)" @dragend="$emit('ondragend', $event)">
    ...
    <div class="card__info">
      <div class="card__attack">atk: {{ card.stats.atk }}</div>
      <div class="card__health">hp: {{ card.stats.hp }}</div>
      <div class="card__speed">spd: {{ card.stats.spd }}</div>
      <div class="card__distance">dis: {{ card.stats.dis }}</div>
    </div>
  </div>
</template>
